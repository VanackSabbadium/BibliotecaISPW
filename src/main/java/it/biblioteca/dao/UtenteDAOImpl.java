...
/**
 * Implementazione JDBC base di UtenteDAO.
 * Usa it.biblioteca.db.DatabaseConfig.getConnection() per ottenere la Connection.
 *
 * ATTENZIONE: adatta getConnection() se la tua utility ha nome diverso.
 */
public class UtenteDAOImpl implements UtenteDAO {
    ...
    public boolean creaCredenziali(Long utenteId, String username, String passwordPlain) throws Exception {
        ...
        try (Connection conn = getConn(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            // fallback se password_plain non esiste
            if (ex.getErrorCode() == 1054 || ex.getSQLState().startsWith("42")) {
                ...
                try (Connection conn = getConn(); PreparedStatement ps2 = conn.prepareStatement(sql2)) {
    ...
    private Utente mapRow(ResultSet rs) throws SQLException {
        ...
        try { u.setUsername(rs.getString("username")); } catch (SQLException ignore) {}
        try { u.setPassword(rs.getString("password_plain")); } catch (SQLException ignore) {}
        return u;
    }
}
